...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("KERNIXCORD_DEV_INSTALL") == "1"
	...
	KernixcordFile := KernixcordDirectory

	stat, err := os.Stat(KernixcordFile)
	if err != nil {
		return
	}
	...
	// dev
	if stat.IsDir() {
		KernixcordFile = path.Join(KernixcordFile, "main.js")
	}

	// Check hash of installed version if exists
	b, err := os.ReadFile(KernixcordFile)
	if err != nil {
		return
	}

	Log.Debug("Found existing Kernixcord Install. Checking for hash...")

	re := regexp.MustCompile(`// Kernixcord (\w+)`)
...
func installLatestBuilds() (retErr error) {
	...
	out, err := os.OpenFile(KernixcordDirectory, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		Log.Error("Failed to create", KernixcordDirectory+":", err)
		retErr = err
		return
	}
	read, err := io.Copy(out, res.Body)
	if err != nil {
		Log.Error("Failed to download to", KernixcordDirectory+":", err)
		retErr = err
		return
	}
	...
	_ = FixOwnership(KernixcordDirectory)
