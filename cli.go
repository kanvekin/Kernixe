...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Kernixcord")
	var updateFlag = flag.Bool("repair", false, "Repair Kernixcord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Kernixcord")
	...
	if *versionFlag {
		fmt.Println("Kernixe Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2026 Vendicated, thororen1234, Vencord, and Kernixcord contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Kernixe' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest Kernixcord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Kernixcord will likely not work.")
}
